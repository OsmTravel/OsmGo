...
export class OpeningHoursComponent implements OnInit {
  ...
  findIntervalsByDay(intervals, day) {
    const intervalsByDay = intervals.filter(interval => interval && interval._dayStart == day)
  ...
  deleteCurrentInterval( _dataRange, _interval){
    ...
    const currentDataRange = this.intervals.find( dr => dr._wideInterval._type = _wideIntervalType)

    const intervals2 = currentDataRange.getTypical().getIntervals();

    const currentIntervalIndex = intervals2.findIndex( int => 
      int &&
  ...
  addIntervals(times, days){
    ...
    for (const dayIndex of daysIndex){
      for (const time of times){
        ...
        currentDataRange.getTypical().addInterval(new Interval(dayIndex,dayEnd,start,end) );
      }
