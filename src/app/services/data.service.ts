...
export class DataService {
    ...
    async applyUploadResults(
        ...
        const osmState = this.createOsmStateSnapshot(
            nextGeojson,
            nextGeojsonChanged
        )
        await Promise.all([
            ...
            this.localStorage.set(OSM_STATE_STORAGE_KEY, osmState),
        ])
        this._geojson = nextGeojson
        this._geojsonChanged = nextGeojsonChanged
        this._osmStateRevision = osmState.revision
        this.notifyChangedData()
    }
    ...
    private createOsmStateSnapshot(
        officialById = this._geojson,
        pendingById = this._geojsonChanged
    ): PersistedOsmStateV2 {
        return {
            schemaVersion: OSM_STATE_SCHEMA_VERSION,
            revision: this._osmStateRevision + 1,
            officialById: cloneDeep(officialById),
            pendingById: cloneDeep(pendingById),
    ...
    private async persistOsmState(): Promise<void> {
