...
//ACCESSORS
	...
	getWeekdays() {
		...
		if(wd.length > 0 && wd.contains(6) && wd.contains(0) && (wd.contains(5) || wd.contains(1))) {
			...
			let startWE = 6;
			let i=wd.length-2, stopLooking = false;
			...
			if(length >= 3 && startWE > endWE) {
				...
				result += Constants.OSM_DAYS[startWE]+"-"+Constants.OSM_DAYS[endWE];
		...
		if(wd.length > 1 || (wd.length == 1 && wd[0] != -1)) {
			result += (result.length > 0) ? ","+Constants.OSM_DAYS[wd[0]] : Constants.OSM_DAYS[wd[0]];
			let firstInRow = wd[0];
