...
//ACCESSORS
	...
	getTimeForHumans() {
		...
		switch(this._type) {
			case "day":
				if(this._end != null) {
					result = "every week from "+Constants.IRL_MONTHS[this._start.month-1]+" "+this._start.day+" to ";
					if(this._start.month != this._end.month) { result += Constants.IRL_MONTHS[this._end.month-1]+" "; }
					result += this._end.day;
				}
				else {
					result = "day "+Constants.IRL_MONTHS[this._start.month-1]+" "+this._start.day;
				}
				break;
			...
			case "month":
				if(this._end != null) {
					result = "every week from "+Constants.IRL_MONTHS[this._start.month-1]+" to "+Constants.IRL_MONTHS[this._end.month-1];
				}
				else {
					result = "every week in "+Constants.IRL_MONTHS[this._start.month-1];
				}
				break;
	...
	getTimeSelector() {
		...
		switch(this._type) {
			case "day":
				result = Constants.OSM_MONTHS[this._start.month-1]+" "+((this._start.day < 10) ? "0" : "")+this._start.day;
				if(this._end != null) {
					//Same month as start ?
					if(this._start.month == this._end.month) {
						...
						result += "-"+Constants.OSM_MONTHS[this._end.month-1]+" "+((this._end.day < 10) ? "0" : "")+this._end.day;
					}
				}
				break;
		...
		if(this.equals(o)) {
			...
			if(o._type == "day") {
				...
				if(o._start.month > this._start.month || (o._start.month == this._start.month && this._start.day == 1)) {
					//Ending before
					if(o._end != null && this._end != null && (o._end.month < this._end.month || (o._end.month == this._end.month && this._end.day == Constants.MONTH_END_DAY[end.month-1]))) {
						result = true;
					}
				...
				if(o._end != null) {
					if(this._end == null) {
						if(
							...
							&& ((o._start.day >= 1 && o._end.day < Constants.MONTH_END_DAY[o._start.month-1])
							|| (o._start.day > 1 && o._end.day <= Constants.MONTH_END_DAY[o._start.month-1]))
						) {
							result = true;
						}
						...
						if(o._start.month >= this._start.month && o._end.month <= this._end.month) {
							if(
								...
								|| (o._start.month > this._start.month && o._end.month == this._end.month && o._end.day < Constants.MONTH_END_DAY[o._end.month-1])
								|| (o._start.day >= 1 && o._end.day < Constants.MONTH_END_DAY[o._end.month-1])
								|| (o._start.day > 1 && o._end.day <= Constants.MONTH_END_DAY[o._end.month-1])
							) {
								result = true;
							}
