...
class DateRange {
    ...
    updateRange(wide) {
        ...
        if (this._typical == undefined) {
            switch (this._wideInterval.getType()) {
                ...
                default:
                    throw Error(
                        'Invalid interval type: ' + this._wideInterval.getType()
                    )
            }
        }
    }
    ...
    hasSameTypical(dr) {
        return (
            this.definesTypicalDay() == dr.definesTypicalDay() &&
            this._typical.sameAs(dr.getTypical())
        )
    }
    ...
    isGeneralFor(dr) {
        return (
            dr.definesTypicalDay() == this.definesTypicalDay() &&
            this._wideInterval.contains(dr.getInterval())
        )
    }
}
