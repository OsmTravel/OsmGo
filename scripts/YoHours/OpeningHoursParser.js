...
class OpeningHoursParser {
    ...
    parse(oh) {
        ...
        for (let i = 0, li = blocks.length; i < li; i++) {
            ...
            if (
                currentToken >= 0 &&
                this._isRuleModifier(tokens[currentToken])
            ) {
            ...
            if (timeSelector == '24/7') {
                ...
                currentToken >= 0 &&
                this._isWeekday(tokens[currentToken])
            ) {
                ...
                for (
                    ...
                    //Weekday interval
                        ...
                        throw new Error(
                            'Invalid weekday interval: ' + singleWeekday
                        )
                    }
                }
            ...
            if (currentToken >= 0) {
                ...
                if (wideRangeSelector.length > 0) {
                    wideRangeSelector = wideRangeSelector
                        .replace(/\:$/g, '')
                        .split('week') //0 = Month or SH, 1 = weeks
                    ...
                    if (monthSelector != null && weekSelector != null) {
                        throw new Error(
                            'Unsupported simultaneous month and week selector'
                        )
                        ...
                        for (
                            ...
                            //Month intervals
                                ...
                                monthFrom =
                                    Constants.OSM_MONTHS.indexOf(
                                        singleMonth[0]
                                    ) + 1
                                if (monthFrom < 1) {
                                    throw new Error(
                                        'Invalid month: ' + singleMonth[0]
                                    )
                                }
                            ...
                            //Unsupported
                                ...
                                throw new Error(
                                    'Unsupported month selector: ' + singleMonth
                                )
                            }
                        }
            ...
            if (months.length > 0) {
                for (let mId = 0, ml = months.length; mId < ml; mId++) {
                    ...
                    if (singleMonth.holiday != undefined) {
                        dateRanges.push(
                            new WideInterval().holiday(singleMonth.holiday)
                        )
                    } else if (singleMonth.fromDay != undefined) {
                        if (singleMonth.toDay != null) {
                            dateRange = new WideInterval().day(
                                singleMonth.fromDay.day,
                                singleMonth.fromDay.month,
                                singleMonth.toDay.day,
                                singleMonth.toDay.month
                            )
                        } else {
                            dateRange = new WideInterval().day(
                                singleMonth.fromDay.day,
                        ...
                        if (singleMonth.to != null) {
                            dateRange = new WideInterval().month(
                                singleMonth.from,
                                singleMonth.to
                            )
                        } else {
            ...
            //Week range
                ...
                for (let wId = 0, wl = weeks.length; wId < wl; wId++) {
                    if (weeks[wId].to != null) {
                        dateRange = new WideInterval().week(
                            weeks[wId].from,
                            weeks[wId].to
                        )
                    } else {
            ...
            if (weekdays.length == 0) {
                if (
                    holidays.length == 0 ||
                    (holidays.length == 1 && holidays[0] == 'SH')
                ) {
                    weekdays.push({
                        from: 0,
                        to: Constants.OSM_DAYS.length - 1,
                    })
                } else {
            ...
            for (let drId = 0, drl = dateRanges.length; drId < drl; drId++) {
                ...
                while (resDrId < result.length && !foundDateRange) {
                    if (
                        result[resDrId].getInterval().equals(dateRanges[drId])
                    ) {
                        foundDateRange = true
                    } else {
                        resDrId++
                ...
                if (foundDateRange) {
                    ...
                    for (resDrId = 0; resDrId < result.length; resDrId++) {
                        if (
                            result[resDrId].isGeneralFor(
                                new DateRange(dateRanges[drId])
                            )
                        ) {
                            general = resDrId
                        }
                    ...
                    if (general >= 0 && drObj.definesTypicalWeek()) {
                        drObj
                            .getTypical()
                            .copyIntervals(
                                result[general].getTypical().getIntervals()
                            )
                    }
                ...
                for (let wdId = 0, wdl = weekdays.length; wdId < wdl; wdId++) {
                    ...
                    if (weekdays[wdId].from <= weekdays[wdId].to) {
                        for (
                            let wdRm = weekdays[wdId].from;
                            wdRm <= weekdays[wdId].to;
                            wdRm++
                        ) {
                            if (drObj.definesTypicalWeek()) {
                                drObj
                                    .getTypical()
                                    .removeIntervalsDuringDay(wdRm)
                            } else {
    ...
    _removeIntervalWd(typical, times, wd) {
        ...
        if (times.to >= times.from) {
            typical.removeInterval(new Interval(wd, wd, times.from, times.to))
        }
        //Interval during night
            ...
            //Sunday
                ...
                typical.removeInterval(new Interval(0, 0, 0, times.to))
            }
        }
    }
    ...
    _addInterval(typical, weekdays, times) {
        ...
        if (typical instanceof Day) {
            if (
                weekdays.from != 0 ||
                (weekdays.to != 0 && times.from <= times.to)
            ) {
    ...
    _addIntervalWd(typical, times, wd) {
        //Interval during day
        if (times.to >= times.from) {
            typical.addInterval(new Interval(wd, wd, times.from, times.to))
        }
        //Interval during night
        else {
            //Everyday except sunday
            ...
            //Sunday
                ...
                typical.addInterval(new Interval(wd, wd, times.from, 24 * 60))
                typical.addInterval(new Interval(0, 0, 0, times.to))
            }
        }
    }
            ...
            for (let i = 0; i < intervals.length; i++) {
                if (intervals[i] == undefined) {
                    ...
                    console.log(
                        i +
                            ': ' +
                            intervals[i].getStartDay() +
                            ', ' +
                            intervals[i].getEndDay() +
                            ', ' +
                            intervals[i].getFrom() +
                            ', ' +
                            intervals[i].getTo()
